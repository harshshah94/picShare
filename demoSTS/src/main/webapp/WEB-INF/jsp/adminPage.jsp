...
<c:if test="${empty allUsers}">
    <h2><small class="text-muted">You </small>don't <small class="text-muted"> have any</small> facebook users <small class="text-muted"> registered in </small><i>picShare!!!</i></h2>
</c:if>
    ...
    <table class="table table-dark">
		...
		<tbody>
			<c:forEach items="${allUsers }" var="user">
				<tr>
				<c:url value="/viewUserPosts" var="myURL">
   						<c:param name="userId" value="${user.userId}" />
					</c:url>
					<td>
						<a href="${myURL}">
							<img alt="Profile pic" src="${user.profilePicLink}" class="img-thumbnail" width="150">
						</a>
					</td>
					<td>
						<a href="${myURL }">
